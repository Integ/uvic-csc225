...
public class Lab4Tester {
    ...
    public static void testList() {
        ...
        //
        StudentList l0 = new StudentLinkedList();
        Student s0  = new Student("abc", 50);
        Student s1  = new Student("def", 56);
        l0.add(s0);
        l0.add(s1);
        displayResults(l0.size() == 2, "test StudentList with size");
        displayResults(l0.toString().equals("abc:50\ndef:56\n"), "test StudentList with toString");

        l0.removeFront();
        displayResults(l0.size() == 1, "test StudentList with size");
        displayResults(l0.toString().equals("def:56\n"), "test StudentList with toString");

        displayResults(l0.contains(s1) == true, "test StudentList with contains");
        displayResults(l0.contains(s0) == false, "test StudentList with contains");
