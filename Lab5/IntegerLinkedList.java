public class IntegerLinkedList {
    ...
    private void doubleElement(IntegerNode n) {
        if (n==null) {
            return;
        } else {
            n.setElement(n.getElement() * 2);
            doubleElement(n.next);
        }
    }
    ...
    private void doubleOdd(IntegerNode n) {
        if (n==null) {
            return;
        } else {
            if (n.getElement() % 2 != 0) {
    ...
    public boolean isAcending() {
        return isAcending(head);
    }
    
    private boolean isAcending(IntegerNode n) {
    ...
    public boolean isNegtive() {
        return isNegtive(head);
    }
    
    private boolean isNegtive(IntegerNode n) {
        if (n==null) {
            return true;
        } else {
            if (n.getElement() >= 0) {
