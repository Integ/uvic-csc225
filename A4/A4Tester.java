public class A4Tester {
	...
	public static void testStackOperations() {
		...
		displayResults(defaultSizeStack.isFull(), "stack now full");
		displayResults(result==5, "top works after second push");
		
		result = defaultSizeStack.pop();
		displayResults(!defaultSizeStack.isEmpty(), "stack not empty");
		displayResults(!defaultSizeStack.isFull(), "stack no longer full");
		displayResults(result==5, "correct value returned from pop");	
		result = defaultSizeStack.top();
		...
		result = defaultSizeStack.pop();
		result = defaultSizeStack.pop();
		displayResults(defaultSizeStack.isEmpty(), "stack is empty again");
		displayResults(!defaultSizeStack.isFull(), "stack is not full");
		displayResults(result==2, "correct value returned from pop");

		defaultSizeStack.push(9);
		result = defaultSizeStack.top();
		displayResults(!defaultSizeStack.isEmpty(), "stack is no longer empty");
		displayResults(!defaultSizeStack.isFull(), "stack is not full");
		displayResults(result==9, "correct value returned from top");	
